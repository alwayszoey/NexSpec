...
export function AdminPortalPage({
  ...
  const [isUnlocked, setIsUnlocked] = useState<boolean>(() => {
    ...
    const isMaster = email === "cpjustink@gmail.com";
  ...
  }, [isUnlocked, fetchMetrics, fetchAdminResources, fetchAdminCategories, fetchSettings, fetchUsers]);
  ...
  const handleSecurityUnlock = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const isMasterEmail = email === "cpjustink@gmail.com" || (currentUser?.email?.toLowerCase() === "cpjustink@gmail.com");
      const isValidPin = pin === DEFAULT_MASTER_PIN || pin === "ADMIN" || pin === "ZORIX";
      const isValid = isMasterEmail && isValidPin;

      if (isValid) {
        setIsUnlocked(true);
        sessionStorage.setItem("admin_portal_unlocked", "true");
        showToast("success", "ปลดล็อกเข้าสู่ระบบจัดการหลังบ้าน Master Admin เรียบร้อย");
      } else {
        setSecurityError("สิทธิ์ปฏิเสธ: แดชบอร์ดนี้อนุญาตเฉพาะบัญชี Master (cpjustink@gmail.com) และต้องระบุ PIN ให้ถูกต้อง");
      }
  ...
  // If locked, render the sleek Master PIN gate
  if (!isUnlocked) {
    return (
      <div className="min-h-screen w-full bg-bg-app text-text-main flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-card-bg rounded-[32px] border border-border-subtle p-8 shadow-2xl relative overflow-hidden">
          <div className="w-12 h-12 rounded-2xl bg-brand/10 text-brand border border-brand/20 flex items-center justify-center mb-6 mx-auto">
            <KeyRound className="w-6 h-6" />
          </div>

          <h2 className="text-xl font-bold text-center text-text-main mb-1">Admin Portal Gateway</h2>
          <p className="text-xs text-center text-text-muted mb-6">
            กรุณาระบุอีเมลผู้ดูแลระบบและ Master PIN เพื่อเข้าถึงหลังบ้าน
          </p>

          {securityError && (
            <div className="mb-4 p-3 bg-danger/10 border border-danger/20 rounded-xl text-xs text-danger flex items-center gap-2">
          ...
          <form onSubmit={handleSecurityUnlock} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-text-muted mb-1">อีเมลผู้ดูแลระบบ (Master Account)</label>
              <input
                type="email"
                required
                ...
                className="w-full px-4 py-2.5 bg-bg-app border border-border-subtle rounded-xl text-sm text-text-main focus:outline-none focus:border-brand"
              />
            </div>

            <div>
              <label className="block text-xs font-semibold text-text-muted mb-1">Master PIN</label>
              <input
                type="password"
                required
                placeholder="ZORIX-9921"
                value={securityPin}
                onChange={(e) => setSecurityPin(e.target.value)}
                className="w-full px-4 py-2.5 bg-bg-app border border-border-subtle rounded-xl text-sm text-text-main focus:outline-none focus:border-brand font-mono"
              />
            </div>
            ...
            <button
              ...
              className="w-full py-3 bg-brand text-white font-bold rounded-xl text-sm shadow-md hover:brightness-110 transition-all cursor-pointer flex items-center justify-center gap-2 mt-2"
            >
          ...
          <div className="mt-6 pt-4 border-t border-border-subtle text-center">
            <button
              onClick={() => navigate("/")}
              className="text-xs text-text-muted hover:text-brand flex items-center gap-1 mx-auto"
            >
  ...
  return (
    <div className="min-h-screen w-full bg-bg-app text-text-main flex flex-col font-sans">
      {/* Toast Notification */}
      {toastMessage && (
        <div
      ...
      {/* Main Top Header */}
      <header className="sticky top-0 z-40 bg-card-bg/80 backdrop-blur-md border-b border-border-subtle px-4 sm:px-8 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate("/")}
            className="p-2 rounded-xl bg-bg-app hover:bg-border-subtle text-text-muted hover:text-text-main border border-border-subtle transition-all mr-1"
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-lg font-bold text-text-main m-0 leading-tight">Admin Portal Gateway</h1>
              <span className="bg-brand/15 text-brand text-[11px] font-bold px-2 py-0.5 rounded-full border border-brand/30">
                MASTER v3.0
              </span>
            </div>
            <p className="text-xs text-text-muted m-0">ระบบจัดการหลังบ้านแบบครบวงจร</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              ...
              fetchSettings();
              fetchUsers();
              showToast("success", "อัปเดตข้อมูลล่าสุดแล้ว");
            }}
            className="px-3 py-2 rounded-xl bg-bg-app hover:bg-border-subtle text-xs font-semibold text-text-muted hover:text-text-main border border-border-subtle transition-all cursor-pointer flex items-center gap-1.5"
          >
          ...
          <button
            onClick={() => navigate("/")}
            className="px-3.5 py-2 rounded-xl bg-brand text-white text-xs font-semibold hover:brightness-110 shadow-sm transition-all cursor-pointer flex items-center gap-1.5"
          >
      ...
      {/* Main Container */}
      <div className="max-w-[1600px] w-full mx-auto p-4 sm:p-8 flex-1 flex flex-col gap-6">
        
        {/* Navigation Tabs */}
        <nav className="bg-card-bg p-2 rounded-2xl border border-border-subtle shadow-sm flex items-center gap-2 overflow-x-auto hide-scrollbar">
          {[
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all cursor-pointer ${
                  isActive
                    ...
                    : "text-text-muted hover:text-text-main hover:bg-bg-app border border-transparent hover:border-border-subtle"
                }`}
              >
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                {[
                  ...
                  const Icon = st.icon;
                  return (
                    <div key={i} className="bg-card-bg p-5 rounded-2xl border border-border-subtle shadow-sm flex flex-col justify-between">
            ...
            <form onSubmit={handleSaveSettings} className="space-y-6 max-w-4xl">
              <div className="bg-card-bg p-6 rounded-2xl border border-border-subtle shadow-sm space-y-4">
                ...
                <div className="flex items-center gap-3 pt-2">
                  <label className="text-xs font-semibold text-text-muted">หรือกรอก HEX:</label>
                  <input
                    type="text"
              ...
              <div className="bg-card-bg p-6 rounded-2xl border border-border-subtle shadow-sm space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-text-muted mb-1">ชื่อร้าน</label>
                    <input
                      type="text"
                      value={settingsForm.name}
                      onChange={(e) => setSettingsForm({ ...settingsForm, name: e.target.value })}
                      className="w-full px-3.5 py-2.5 bg-bg-app border border-border-subtle rounded-xl text-sm text-text-main"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-semibold text-text-muted mb-1">สโลแกน</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-text-muted mb-1">URL โลโก้</label>
                    <input
                      type="text"
              ...
              <div className="bg-card-bg p-6 rounded-2xl border border-border-subtle shadow-sm space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-base font-bold text-text-main flex items-center gap-2">
                    <Radio className="w-5 h-5 text-amber-500" /> แถบประกาศวิ่ง (Announcement Marquee)
                  </h3>
            ...
            <div className="space-y-4">
              <div className="bg-card-bg p-4 rounded-2xl border border-border-subtle shadow-sm flex items-center justify-between gap-4">
                ...
                <div className="text-xs text-text-muted font-medium">
                  สมาชิก: <span className="font-bold text-text-main">{usersList.length} คน</span>
                </div>
              </div>
                    ...
                    <tbody className="divide-y divide-border-subtle">
                      {filteredUsers.map((u) => (
                        <tr key={u.id || u._id} className="hover:bg-bg-app/50 transition-colors">
                          ...
                          <td className="p-4">{u.provider || "credentials"}</td>
                          <td className="p-4">
              ...
              <div className="bg-card-bg p-6 rounded-2xl border border-border-subtle shadow-sm space-y-4">
                <div className="flex items-center justify-between p-4 rounded-xl bg-bg-app border border-border-subtle">
                  <div>
                    ...
                    <p className="text-xs text-text-muted">รีเซ็ตและล้างข้อมูลที่เก็บไว้ชั่วคราวในแคช</p>
                  </div>
                  <button
        ...
        <div className="fixed inset-0 z-[180] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
          <div className="bg-card-bg text-text-main w-full max-w-[520px] rounded-[28px] border border-border-subtle shadow-2xl p-6 relative">
            ...
            <form onSubmit={handleSaveCategory} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-text-muted mb-1">คำอธิบาย</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-semibold text-text-muted mb-1">URL รูปภาพหน้าปก</label>
                <input
                  type="text"
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-border-subtle">
                <button
                  type="button"
                  onClick={() => setIsCategoryModalOpen(false)}
                  className="px-4 py-2.5 rounded-xl text-sm font-semibold bg-bg-app text-text-muted hover:bg-border-subtle border border-border-subtle"
                >
                  ยกเลิก
                </button>
                <button
          ...
          <div className="bg-card-bg text-text-main w-full max-w-[700px] max-h-[90vh] rounded-[28px] border border-border-subtle shadow-2xl p-6 relative flex flex-col my-auto">
            ...
            <div className="flex items-center gap-3 mb-4 shrink-0">
              ...
              <div>
                ...
                <p className="text-xs text-text-muted m-0">กรอกข้อมูลสินค้าและลิงก์ดาวน์โหลด</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSaveProduct} className="space-y-4 overflow-y-auto pr-1 flex-1">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-text-muted mb-1">ราคา</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-text-muted mb-1">ประเภท</label>
                  <select
                    value={formData.actionType}
                    onChange={(e) => setFormData({ ...formData, actionType: e.target.value as any })}
                    className="w-full px-3.5 py-2.5 bg-bg-app border border-border-subtle rounded-xl text-sm text-text-main focus:outline-none focus:border-brand"
                  >
                    <option value="link">🔗 ลิงก์ดาวน์โหลด</option>
                    <option value="purchase">🛒 ซื้อสินค้า / คีย์</option>
                  </select>
                </div>
              </div>
              ...
              <div className="bg-bg-app/70 p-4 rounded-xl border border-border-subtle space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold text-text-main flex items-center gap-1.5">
                    <Download className="w-4 h-4 text-brand" /> ลิงก์ดาวน์โหลด
                  </label>
                  <button
                    type="button"
                    onClick={() =>
